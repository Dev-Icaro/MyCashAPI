...
const {
   MSG_EMAIL_NOT_FOUND,
   MSG_EMAIL_SUCCESS,
} = require("../constants/config-contants");

class ConfigService {
   static async getEmailConfigById(id) {
      if (!id) return { statusCode: 400, message: ERROR_MISSING_PARAM + "id" };

      let EmailConfig = await dataBase["EmailConfig"].findOne({
         where: { id: Number(id) },
      });

      if (!EmailConfig)
         return { statusCode: 400, message: MSG_EMAIL_NOT_FOUND };
      else return { statusCode: 200, EmailConfig };
   }
